...
本题的各种算法也可以用c++语言来实现，两种语言相比较，我发现由于Python语言的自动回收内存机制和函数式编程特征，以及丰富的内置函数，
使得编程者无需考虑过多的实现细节，能更关注算法思想本身，代码也更简洁，更优雅。
'''
...
def dfs_1(a, t, c): 
    ...
    if check(a): #已经有解就不必再翻转更多棋子了
        ...
        for i in range(len(lib)):
            if c+1 < min_cc and t & lib[i] == 0: #如果没有更优解就剪枝，只在第一个值为1的二进制位左侧设置1，以避免重复
...
def dfs_2(a, d, c):  
    ...
    if check(a): #已经有解就不必再翻转更多棋子了
        ...
        for i in range(len(d)):
            if c+1 < min_cc and d[i] == 0:#如果没有更优解就剪枝，只在第一个值为1的二进制位左侧设置1，以避免重复
                d[i] = 1
                turn(a, i, size)
                dfs_2(a, d, c+1)
                turn(a, i, size) #回溯
